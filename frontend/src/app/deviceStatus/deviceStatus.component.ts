...
export class DeviceStatusComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private cdr: ChangeDetectorRef, //private tabStateService: TabStateService
  ) {}
  ...
  ngOnInit() {
    ...
    this.subscription.add(
      this.connectionStatusService.currentM3MStatus.subscribe(
        (_M3MStatus) => {
      ...
      .subscribe({
        next: (message) => {
          if (
            ...
            this.notificationService.showError(
              'Ошибка отключения от устройства',
            );
            this.buttonControlDevice(device, true, subscription);
            timeout.unsubscribe();
          }
      ...
      case 'StatIP':
        ...
        if ((this.inputIP_ABONENT && this.inputIP_BASE) == '') {
          ...
          this.notificationService.showWarning(
            'Введите IP адрес для станций ',
          );
          return;
        }

        const isBaseValid = this.validateIpAddress(this.inputIP_BASE);
        const isAbonentValid = this.validateIpAddress(
          this.inputIP_ABONENT,
        );

        if (!isBaseValid || !isAbonentValid) {
          this.notificationService.showWarning(
            'Некорректный ввод IP адресов для станций ',
          );
          this.loadingButtons[device] = false;
          return;
        }
      ...
      .subscribe({
        next: (message) => {
          if (message.status === 'sended') {
            ...
            switch (device) {
              case 'StatOID':
                this.connectionStatusService.changeOidParamsStatus(
                  InputedParams['frequency'],
                );
                break;
              case 'StatIP':
                this.connectionStatusService.changeIpParamsStatus(
                  InputedParams['baseIP'],
                  InputedParams['abonentIP'],
                );
                break;
              case 'M3Moffset':
                this.connectionStatusService.changeOffsetM3MStatus(
                  InputedParams['offset'],
                );
                break;
            }
            this.cdr.detectChanges();
