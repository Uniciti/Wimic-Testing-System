...
export class Queue {
  ...
  public async start(): Promise<void> {
    ...
    if (this.running) {
      broadcaster(
        JSON.stringify({ type: "warn", message: "test already running" })
      );
      return;
    }
  ...
  public stop(): void {
    if (!this.running) {
      broadcaster(
        JSON.stringify({ type: "warn", message: "Test queue is not running." })
      );
      return;
    }
    ...
    this.running = false;
    broadcaster(
      JSON.stringify({ type: "stop", message: "stoping test queue" })
    );
  }
