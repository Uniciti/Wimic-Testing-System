    ...
    return new Promise(async (resolve) => {
      ...
      for (const i of this.modList) {
        ...
        dataArray.push({
          ...
          SNR: "none",
          LostPercent: "none",
          TestStatus: "Ошибка поиска модуляции",
        ...
        const message = {
          status: "modulation",
          messageMod: this.modList.findIndex((element) => element === i) + 1,
        ...
        await tcpClient.sendCommand(attValue);
        await delay(2000);
        const x = await snmpClient.getFromSubscriber(
        ...
        if (x == i.toString()) {
          ...
          const errorRate = parseFloat(
            ((lostBytes / txBytes) * 100).toFixed(2)
          );
